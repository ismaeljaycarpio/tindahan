...
export const getAllProducts = async (req, res) => {
  try {
    const products = await Product.find({}); // no filter - find all products
    res.json({ products });
  } catch (error) {
...
export const getFeaturedProducts = async (req, res) => {
  try {
    ...
    if (featuredProducts) {
      return res.json(JSON.parse(featuredProducts)); // convert to obj
    }*/

    // check in MongoDB
    // lean() - returns JS objects instead of mongoDB document
    ...
    // store in Redis for future quicker access
